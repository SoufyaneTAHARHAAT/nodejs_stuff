...
app.get('/jokes/:id', (req, res) => {
  ...
  const chosenJoke = jokes.find(joke => joke.id === jokeId);
  res.json(chosenJoke);
});
...
app.post('/jokes', (req, res) => {
  ...
  jokes.push({id: id, jokeText: jokeText, jokeType: jokeType});
  res.json(jokes[jokes.length-1]);
});

//5. PUT a joke
app.put('/jokes/:id', (req, res) => {
  const oldJoke = jokes.find(joke => joke.id === parseInt(req.params.id));
...
app.delete('/jokes/:id', (req, res) => {
  ...
  const jokeIndex = jokes.findIndex(joke => joke.id === selectedId);
  if(jokeIndex > -1) {
    jokes.splice(jokeIndex, 1);
    res.status(200);
    res.send(`Joke with id ${selectedId} deleted`);
  } else {
    res.status(404).send("No jokes found with that id");
  }
});
